...
if strcmp(field,'data') && ~isa(cfg,'wlanNonHTConfig')
    % Spatial mapping only relavent for:
...
function csd = getCyclicShiftMatrix(cfg,varargin)
    % CSD = getCyclicShoftMatrix(CFG) returns a Nst-by-Nsts-by-1/Ntx matrix
    ...
    %
    % CSD = getCyclicShoftMatrix(CFG,FIELD) the cyclic shift applied to the
    % FIELD. FIELD can be 'preamble' or 'data'.
    %
    % CSD = getCyclicShoftMatrix(CFGMU,RUIDX,FIELD) the cyclic shift
...
%
%   CFG is a format configration object of type wlanHESUConfig,
%   wlanVHTConfig, or wlanHTConfig.
%
%   Q = getSpatialMappingMatrix(CFGMU,RUIDX) returns the spatial mapping
%   matrix for an HE-MU configuation. CFGMU is a wlanHEMUConfig object and
%   RUIDX is the index of the RU of intererst.
